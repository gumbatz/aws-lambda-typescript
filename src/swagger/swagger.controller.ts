...
import { ConfigurationErrorResult, ErrorResult, ForbiddenResult, NotFoundResult }
  from '../../shared/errors';
...
export class SwaggerController {
  public constructor(private service: SwaggerService) {
  }

  public getSwaggerJson: ApiHandler = (
    event: ApiEvent, context: ApiContext, callback: ApiCallback,
  ): void => {
    this.service.getSwaggerDescription()
      .then((result: GetSwaggerResult) => {
        return ResponseBuilder
          .ok<GetSwaggerResult>(result, callback);
      })
