  ...
  describe('getSwaggerJson function', () => {
    describe('success', () => {
      it('should return HTTP 200 OK', async () => {
        when(swaggerServiceMock.getSwaggerDescription())
          .thenReturn(Promise.resolve<GetSwaggerResult>(testData.swaggerDoc));
        const response: ApiResponseParsed<GetSwaggerResult> =
          await callSuccess<GetSwaggerResult>(controller.getSwaggerJson);
    ...
    describe('service failures', () => {
      it('should return Configuration Error for improper configuration', async () => {
        const errorResult: ConfigurationErrorResult =
          new ConfigurationErrorResult(errorCode.GeneralError, 'Sorry...');
    ...
    async function callAndCheckError(
      expectedHttpStatusCode: number, errorResult: ErrorResult,
    ): Promise<void> {
